...
- Day 2–7 自动采证已重新绑定最终 f1 身份：当前密封脚本位于 `/opt/dsa-development/pr6-shadow-observation-f1aeb22b/collect-policy-shadow-observation-f1aeb22b.sh`，SHA-256 为 `9824cee817d49fb440dc83138e66ff909d2ccffa3bc29950ef7d8398a70142ad`；除 12 条迁移、9 个 Outcome、252 个 raw 引用与 170 个 raw 文件闭包外，还逐日记录账户动作、would-block、估值完整性、权重和 Policy/Signal 交叉血缘。替换前脚本与预检证据保留在 `/opt/dsa-development/pr6-shadow-observation-f1aeb22b.collector-blocked-20260811T111400Z-078d785a`，`replacement.json` / `SHA256SUMS` SHA-256 分别为 `93f93155dd927ae5da217f2deb54cb99cd27c827903197927f78b555bc32c7e9` / `1c2c0f2d5eb072ee2d5424c6bfd113f0bb6ee114b25fdc1298d6bda1228c62dd`。
- 六个持久化 `dsa-pr6-shadow-observation-dayN-f1aeb22b-persistent.timer` 分别固定到 `2026-08-12/13/14/17/18/19 15:20 CST`，主机重启后仍会恢复并补跑错过的触发点；旧 93c 与 f1 transient timers 均已停止。初始持久化 unit 证据位于 `/opt/dsa-development/pr6-shadow-observation-f1aeb22b/systemd-units-20260811-f1aeb22b`，`installed-units.json` / `SHA256SUMS` SHA-256 分别为 `7e5e4d94fdb7ab03ad7660ddbec6e4c72f8fb8735553ccfe7254aab4b48e1239` / `8f5a28faa4707b6c12ac1210393a783b1df41f2978916631866f2ef70e075de4`。
- Day 7 后的自动汇总脚本 `aggregate-policy-shadow-g9-f1aeb22b.py` SHA-256 为 `fcfa2390e0d072cd391e14e2de1f924930aa095073eb5afd802fd50551ab3054`，持久化 timer 固定在 `2026-08-19 15:40 CST`。它会逐日复验七份发布树、生产容器/API/DB、Policy/Signal 交叉血缘及 append-only 计数，只生成 `automated_gate=PASS` 与 `human_signoff=PENDING` 的证据，不会自动启用 Enforce。systemd 证据位于 `/opt/dsa-development/pr6-shadow-observation-f1aeb22b/g9-aggregate-systemd-20260811-f1aeb22b`，`installed-units.json` / `SHA256SUMS` SHA-256 分别为 `39736695f9312b8a373eb4d7477ab32f39fe54afc06ffa7e3c9c0f7bd7b6add7` / `76f1d6058543a7f3e88473cb164f311998576e3be5fb0ed0bb9a6ebac0171a55`。真实 Day 1 加六份私有合成 v3 日证据的七天端到端演练已通过且 `/tmp` 全部清理；密封结果位于 `/opt/dsa-development/pr6-shadow-observation-f1aeb22b/g9-aggregate-preflight-20260811-f1aeb22b`，`preflight.json` / `SHA256SUMS` SHA-256 分别为 `fc4cbe77f47e3f8c4258a256040a0ed1cdbbe0ea7142b827d561dabf0d74f458` / `9ab95eed7abc6ceeb017b11bd1e9831cfc0f7454ce69299f817068648f11b19f`。
...
| G9 Policy Shadow | 7 个真实交易日 shadow；差异、误阻断、缺失估值、账户动作和回滚均复核 | `IN_PROGRESS` | Day 1 基线与收盘后观察均已封存；Day 2–7 的 f1 定时采证和 Day 7 自动汇总已持久化武装，2026-08-12 至 19 仍需逐日证据与人工签字 |
