...
| `BOCHA_API_KEYS` | [Bocha Search](https://open.bochaai.com/) Web Search API (Chinese search optimized, supports AI summaries, multiple keys comma-separated) | Optional |
| `BAIDU_AI_SEARCH_API_KEYS` | [Baidu Qianfan AI Search](https://cloud.baidu.com/doc/qianfan-api/s/Hmbu8m06u) API key (Baidu Search v2 for Chinese and A-share web recall) | Optional |
...
| `TUSHARE_PRIORITY` | Tushare daily K-line provider priority (`0-99`, lower runs earlier). Leave blank to preserve token-driven automatic priority; an explicitly invalid value safely falls back to `2`. | blank (automatic) | Optional |
...
| `FUNDAMENTAL_AUXILIARY_TIMEOUT_SECONDS` | Independent timeout for optional auxiliary fundamental sources so slow AkShare blocks cannot consume the full stage budget | `4.0` | Optional |
...
P5 extends the existing Web `/decision-signals` page instead of adding a new navigation page or BacktestPage entry. The filter area shows current outcome-engine stat cards; the details drawer lazily loads outcomes and lets the user submit useful/not useful feedback. In production, the single Scheduler owner runs one v1 Outcome maintenance pass after startup and repeats it every `DECISION_SIGNAL_OUTCOME_INTERVAL_MINUTES`, capped by `DECISION_SIGNAL_OUTCOME_BATCH_LIMIT`. Set `DECISION_SIGNAL_OUTCOME_ENABLED=false` to stop background maintenance; explicit `POST /api/v1/decision-signals/outcomes/run` remains available. Batch maintenance prioritizes missing outcomes, retries recoverable unable rows, and skips non-directional actions and unsupported natural horizons. This v1 maintenance is independent from the default-off `DECISION_OUTCOME_V2_ENABLED` rollout flag.
...
#1390 P7 is documented in [DecisionSignal Topic](decision-signals.md) (Chinese-only). #1756 adds a nullable `decision_profile` column, API request/response field, and profile-aware indexes for `decision_signals`. Existing SQLite databases only run `ALTER TABLE ADD COLUMN` when the column is missing; the migration does not drop/rebuild `decision_signals` and does not delete old indexes. It creates profile-aware indexes idempotently and defensively parses `metadata_json` row by row, backfilling only legal `metadata.decision_profile` values while keeping invalid JSON, non-object metadata, or invalid profiles as `NULL`. `DECISION_SIGNAL_OUTCOME_ENABLED` only provides a soft rollback for v1 background Outcome maintenance; it does not stop signal extraction or writes, so a full rollback still reverts the related code. Report saving, alert triggering, notification sending, and the portfolio risk main flow continue through their existing paths, and historical signal, feedback, and outcome rows are not deleted automatically.
