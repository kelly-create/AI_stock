...
Personal Research retry guarantees begin after a checkpoint has been persisted. Bound Datasets, Debate stances, the final Research Snapshot, all five Skills, Review, Analysis History, DecisionSignal, and Thesis are reused or strictly revalidated, so their upstream stages are not run again. A crash after a provider/LLM response but before its checkpoint commit can still repeat that physical request; the system does not claim physical-request exactly-once. The main analysis LLM checkpoint is the successfully committed Analysis History. Once History exists, terminal resume runs before any Provider, Debate, or main analysis LLM call and completes only a missing deterministic Signal/Thesis tail.

Formal Personal Research does not depend on the general `SAVE_CONTEXT_SNAPSHOT` switch retaining the full diagnostic context. Even when that switch is `false`, History privately stores only the minimum replay block: frozen market phase, Policy mode, Policy version/hash, Gate and server-side Policy-context code fingerprints, and the server-owned portfolio input snapshot. History/Analysis APIs strip this private block from `context_snapshot`. A History-only retry may replay only these frozen inputs and never reads the current runtime Policy mode or Portfolio. A missing or corrupted private block, or a Policy/code contract that differs from the frozen fingerprints, must fail closed instead of rewriting the Signal, Policy audit, or Thesis from current state.
...
Do not copy an active SQLite main file with `cp` or `tar`; committed WAL data may not yet be merged into that file. See the [SQLite online backup, verification, and restore runbook](operations/sqlite-backup.md) (Chinese) for strict verification, restore drills, and production replacement. Before migration, use the currently deployed sealed tool and old-schema table set to create the rollback backup. After migration, use the candidate's current default set and account for the watchlist, reconciliation, budget, policy, Skill/Review/Thesis, and Outcome v2 tables. Do not substitute the new candidate list for the pre-migration evidence, and do not accept a post-migration backup with the legacy table set.
