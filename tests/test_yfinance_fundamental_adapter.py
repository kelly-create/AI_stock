...
def _recent_dividend_index() -> pd.DatetimeIndex:
    """Return four stable ex-dates that are all inside the rolling TTM window."""
    today = pd.Timestamp.now(tz="America/New_York").normalize()
    return pd.DatetimeIndex(
        [today - pd.Timedelta(days=days) for days in (300, 210, 120, 30)]
    )
...
class TestYfinanceFundamentalAdapter(unittest.TestCase):
    def test_populates_growth_earnings_dividend_boards_for_us_stock(self) -> None:
        ...
        dividend_index = _recent_dividend_index()
        dividends = pd.Series(
            [0.26, 0.26, 0.26, 0.27],
        ...
        self.assertEqual(
            ...
            dividend_index[-1].date().isoformat(),
        )
